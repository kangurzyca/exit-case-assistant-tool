...
var FetchedData = /** @class */ (function () {
    function FetchedData(name, type, regexRule) {
        this.name = name;
        this.type = type;
...
fetchedDataRequirements.push(new FetchedData("Case Number", "caseNumber", /EXT\d{10}/i));
var filteredData = [];
function pasteText() {
    ...
    navigator.clipboard
        ...
        fetchedDataRequirements.forEach(function (el) {
            var result = el.lookFor(inputString);
            var temp = {
                name: el.name,
                type: el.type,
                data: result,
